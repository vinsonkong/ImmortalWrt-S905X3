...
# ==========================================
# 优化版：覆盖式还原配置 (Optimized Config Restore)
# =======================================

BACKUP_SRC="/mnt/mmcblk2p4/backup"

echo ">>> 正在使用已挂载的目录 ${TARGET_ROOT_MP} 进行配置还原..."



# 2. 执行配置更新（合并 + 覆盖）
if [ -d "${BACKUP_SRC}" ] && [ "$(ls -A ${BACKUP_SRC} 2>/dev/null)" ]; then
    echo ">>> 检测到备份目录: ${BACKUP_SRC}"
    echo ">>> 开始合并并覆盖更新配置..."
    
    # 遍历备份目录下的所有文件和文件夹 (如 etc, usr, www)
    for item in "${BACKUP_SRC}"/*; do
        # 获取文件夹名称，例如 "etc"
        item_name=$(basename "${item}")
        target_path="${TARGET_ROOT_MP}/${item_name}"
        
        # 确保目标位置的父目录存在
        mkdir -p "${target_path}"
        
        echo ">>> 正在覆盖更新 ${item_name} ..."
        
        # 使用 cp -af 进行合并+覆盖复制
        # -a: 归档模式，保留权限/时间戳/链接等所有属性
        # -f: 强制覆盖已存在的文件
        cp -af "${item}/." "${target_path}/"
    done
    
    echo ">>> ✅ 所有配置覆盖更新完成"
else
    echo ">>> ⚠️ 未找到有效备份配置 (${BACKUP_SRC})，跳过还原"
fi
